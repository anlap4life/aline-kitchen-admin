let orderDetailApp = function () {
    ...
    function initOrderTable() {
        new DataTable('#order-detail-table', {
            ...
            columns: [
                ...
                {
                    data: 'paymentSubtotal', render: function (data) {
        ...
        $('#order-detail-table tbody').on('click', 'button.hapus', function (e) {
            var d = $('#order-detail-table').DataTable()
                .row(e.target.closest('tr')).data();
    ...
    function addOrderFormHandler() {
        ...
        $('#qty-input').on('blur', function (e) {
            ...
            addOrderForm[2].value = formatRupiah(chosenMenu.price * parseInt(this.value));
        })
